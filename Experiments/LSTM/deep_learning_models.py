...
def transformer_dense_layer(maximum_position_encoding, embed_dim, num_heads, num_classes, l2):
    dff = 256
    ...
    x = pos_embedding_layer(x)
    x = encoder_layer_1(x)
    x = encoder_layer_2(x)
    x = layers.GlobalAveragePooling1D()(x)
    outputs = layers.Dense(num_classes, activation="softmax", kernel_regularizer=keras.regularizers.l2(l2))(x)
...
def lstm_model_dense_layer(num_classes, embed_dim, l2):
    inputs = layers.Input(shape=(None, 1))

    x = layers.Dense(embed_dim, activation='relu')(inputs)  # 注意这里激活函数可选
